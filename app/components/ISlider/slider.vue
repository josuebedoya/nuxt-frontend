...
import {sliderDefaults} from './partials/defaults'
...
const dotsInContent = computed(() => props.dotsConfig?.inContent)
...
const ubicationDotsClass = computed(() => [
 'min-w-0 overflow-hidden max-[768px]:col-span-full items-center relative',
 (!dotsInContent.value && dotsPosition.value === 'right') && 'md:grid md:grid-cols-[1fr_max-content]',
 (!dotsInContent.value && dotsPosition.value === 'left') && 'md:grid md:grid-cols-[max-content_1fr]'
].filter(Boolean))
const dotsClass = computed(() => [
 'shrink-0',
 dotsInContent.value && 'absolute z-10 flex justify-center items-center',
 ['right', 'left'].includes(dotsPosition.value) && 'h-full top-0 bottom-0',
 dotsPosition.value === 'left' && 'left-0',
 dotsPosition.value === 'right' && 'right-0',
 ['top', 'bottom'].includes(dotsPosition.value) && 'w-full left-0 right-0',
 dotsPosition.value === 'top' && 'top-0',
 dotsPosition.value === 'bottom' && 'bottom-0',
].filter(Boolean))
   ...
   <div :class="['carousel', { vertical: config?.isVertical }]">
    ...
    <div :class="primaryCarouselClass">
     ...
     <div :class="ubicationDotsClass">
      ...
      <!-- DOTS - TOP / LEFT-->
      <div v-if="withDots && ['top', 'left'].includes(dotsPosition)" :class="dotsClass">
      ...
      <!--  DOTS - BOTTOM / RIGHT-->
      <div v-if="withDots && ['bottom', 'right'].includes(dotsPosition)" :class="dotsClass">
       <Dots v-bind="propsDots"/>
      </div>
     </div>
