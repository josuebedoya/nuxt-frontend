...
const isValidSize = variables.sizes.includes(props.size)
const isValidColor = variables.colors.includes(props.color)

const classDot = computed(() => [
 'dot',
 isValidSize ? '!py-0 px-[2px]' : `${props.size} leading-none`,
...
const indexActive = computed(() => {
 if (typeof props.activeDot === 'object' && 'value' in props.activeDot) {

  return props.activeDot.value
 }

 return props.activeDot ?? 0
})
</script>

<template>
 <div
   class="dots flex justify-center items-center gap-3 my-3"
   :class="[containerClass, { 'flex-col mx-3': vertical }]"
 >
